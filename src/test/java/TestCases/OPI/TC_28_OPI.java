...
public class TC_28_OPI {
	...
	public boolean Test(Reports report, DriverManager DM, int iteration, String name, String configEntidad, String cuentas_generales) {
		...
		try {
			...
			System.out.println("Configuring TC_28_OPI\r\n");
			...
			//Seteo el producto para que no opere adelantos
			rtaQuery = oraResp.oraUpdate(report, "UPDATE PRODUCTOS SET OPERA_ADELANTOS = 0 WHERE ID_PRODUCTO = " + ID_PRODUCTO , configEntidad);
			ppCondi.preCondicionBD(report, rtaQuery);
			
			//EJECUCION OPI
			res = opiCmd.sshSendCmd(report, "TC_28_OPI.xml", configEntidad);
			...
			//Seteo el producto para que opere adelantos
			rtaQuery = oraResp.oraUpdate(report, "UPDATE PRODUCTOS SET OPERA_ADELANTOS = 1 WHERE ID_PRODUCTO = " + ID_PRODUCTO , configEntidad);
			ppCondi.preCondicionBD(report, rtaQuery);
