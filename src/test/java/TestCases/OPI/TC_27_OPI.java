...
public class TC_27_OPI {
	...
	public boolean Test(Reports report, DriverManager DM, int iteration, String name, String configEntidad, String cuentas_generales) {
		...
		try {	
			...
			System.out.println("Configuring TC_27_OPI\r\n");
			...
			//Seteo el producto para que no opere internacional
			rtaQuery = oraResp.oraUpdate(report, "UPDATE PRODUCTOS SET OPERA_INTERNACIONAL = 0 WHERE ID_PRODUCTO = " + ID_PRODUCTO , configEntidad);
			ppCondi.preCondicionBD(report, rtaQuery);
			
			//EJECUCION OPI
			res = opiCmd.sshSendCmd(report, "TC_27_OPI.xml", configEntidad);
			...
			rtaQuery = oraResp.oraUpdate(report, "UPDATE PRODUCTOS SET OPERA_INTERNACIONAL = 1 WHERE ID_PRODUCTO = " + ID_PRODUCTO , configEntidad);
			ppCondi.preCondicionBD(report, rtaQuery);
