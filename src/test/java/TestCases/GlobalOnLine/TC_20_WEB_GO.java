...
import Pasos.GlobalOnLine.TC_20_GO_PASOS;

public class TC_20_WEB_GO {
	WebDriver driver;
	...
	public boolean Test(Datasources data,Reports report, DriverManager DM, int iteration,String name, String configEntidad, String cuentas_generales) {
		...
		try {	
			...
			System.out.println("Configuring TC_20_WEB_GO\r\n");
			...
			TC_20_GO_PASOS tc = new TC_20_GO_PASOS();
			...
			if (resp[0].equals("2") && resp[1].equals("0")) {
				report.AddLine("Validacion correcta de ID_ESTADO y SOPORTE_FISICO");
				System.out.println("##[section] :  Validacion correct de ID_ESTADO y SOPORTE_FISICO \r\n");
			...
			//ROLLBACK
			rta1 = oraResp.oraUpdate(report, "UPDATE TARJETAS SET ID_ESTADO = 0 WHERE ID_CUENTA = " + ID_CUENTA + " AND ID_TARJETA =" + ID_TARJETA, configEntidad);
			rta2 = oraResp.oraDelete(report, "DELETE TARJETAS_HISTORIAL WHERE ID_TARJETA IN (SELECT ID_TARJETA FROM TARJETAS WHERE ID_CUENTA = " + ID_CUENTA + " AND ID_TARJETA NOT IN (" + ID_TARJETA + "))", configEntidad);
			rta3 = oraResp.oraUpdate(report, "UPDATE SOCIOS SET ID_TARJETA_VIGENTE = NULL WHERE ID_CUENTA = " + ID_CUENTA, configEntidad);
			rta4 = oraResp.oraDelete(report, "DELETE TARJETAS WHERE ID_CUENTA = " + ID_CUENTA + " AND ID_TARJETA NOT IN (" + ID_TARJETA + ")", configEntidad);
			rta5 = oraResp.oraUpdate(report, "UPDATE TARJETAS SET ID_ESTADO = 0 WHERE ID_CUENTA = " + ID_CUENTA + " AND ID_TARJETA =" + ID_TARJETA, configEntidad);
			rta6 = oraResp.oraUpdate(report, "UPDATE SOCIOS SET ID_TARJETA_VIGENTE = " + ID_TARJETA_VIGENTE + " WHERE ID_CUENTA =" + ID_CUENTA, configEntidad);
			
			
			if (rta1 > 0 && rta2 > 0 && rta3 > 0 && rta4 > 0 && rta5 > 0 && rta6 > 0) {
				report.AddLine("Rollback exitoso! Se realizaron los borrados y update correctamente. ");
				System.out.println("##[section] : Rollback exitoso! Se realizaron los borrados y update correctamente.  \r\n");
			}	else {
